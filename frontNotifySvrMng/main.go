...
var gReqQueryFrontNotifySvrCounter int64
//rsp计数器，为线程安全请用atomic.AddUint64()进行操作
var gRspQueryFrontNotifySvrCounter int64
...
func main(){
	...
	if err != nil{
		rlog.Fatalf("start scaleBreak failed.[%+v]", err)
	}
	...
	pb.RegisterFrontNotifySvrMngServer(grpcServer, &serverT{})
...
func statReportRoutine(){
	for {
		...
		{
			data := rlog.StatPointData{Name:"reqQueryFrontNotifySvrCount", Data:gReqQueryFrontNotifySvrCounter}
			dataArr = append(dataArr, data)
			data = rlog.StatPointData{Name:"rspQueryFrontNotifySvrCount", Data:gRspQueryFrontNotifySvrCounter}
			dataArr = append(dataArr, data)
		}
...
func (s *serverT) QueryMyFrontNotifySvr(ctx context.Context, in *pb.CClientAuthInfo) (*pb.CFrontNotifySvrAddr, error) {
	//统计计数
	atomic.AddInt64(&gReqQueryFrontNotifySvrCounter, 1)
	...
	if svrAddr == "" {
		return &pb.CFrontNotifySvrAddr{Addr:""}, errors.New("no available server")
	}

	//统计计数
	atomic.AddInt64(&gRspQueryFrontNotifySvrCounter, 1)
	return &pb.CFrontNotifySvrAddr{Addr:svrAddr}, nil
}
//gRpc proto end///////////////////////////////////////////
...
func serviceCheck(svrAddr string) error{
	...
	defer conn.Close()
	client := pb.NewFrontNotifySvrClient(conn)

	_, err = client.CheckAvail(context.TODO(), &pb.FrontNotifyCheckAvailReq{})
	return err
}
